...
export function EmailAuth() {
  ...
  return <section className="mx-auto w-full max-w-[480px]">
    ...
    {mode !== "reset" && <>
      <button type="button" disabled={busy || !client} onClick={() => void account.signIn(new URLSearchParams(location.search).get("next") ?? "/account")} className="flex h-14 w-full cursor-pointer items-center justify-center gap-3 rounded-xl border border-border bg-muted/40 px-4 text-base font-medium transition-colors hover:bg-muted disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50 sm:h-16 sm:text-lg">
    ...
    <form onSubmit={submit} className="space-y-5">
      ...
      {mode !== "reset" && <div className="space-y-2.5">
        ...
        <div className="relative">
          ...
          <button type="button" aria-label={showPassword ? "Hide password" : "Show password"} aria-pressed={showPassword} onClick={() => setShowPassword(value => !value)} className="absolute inset-y-0 right-0 flex w-14 cursor-pointer items-center justify-center rounded-r-xl text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{showPassword ? <EyeOff className="size-5" /> : <Eye className="size-5" />}</button>
        </div>
        {mode === "signup" && <p id="password-hint" className="text-xs text-muted-foreground">At least 8 characters</p>}
        {mode === "signin" && <div className="flex justify-end"><button type="button" disabled={busy} onClick={() => changeMode("reset")} className="h-9 cursor-pointer rounded-lg border border-border bg-muted/40 px-3 text-sm font-medium text-foreground transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50">Forgot password?</button></div>}
      </div>}
      ...
      <button type="submit" disabled={busy || !client} className="h-14 w-full cursor-pointer rounded-xl bg-foreground px-4 text-lg font-semibold text-background shadow-sm transition-colors hover:bg-foreground/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50">{busy ? "Please wait…" : mode === "signup" ? "Sign up" : mode === "reset" ? "Send reset link" : "Sign in"}</button>
    </form>
    {canResend && <button disabled={busy} onClick={() => void resend()} className="mt-4 cursor-pointer text-sm text-muted-foreground underline underline-offset-4">Resend confirmation email</button>}
    <div className="mt-6 flex items-center justify-center gap-3 text-sm text-muted-foreground"><span>{mode === "signup" ? "Already have an account?" : mode === "signin" ? "Don’t have an account?" : "Remember your password?"}</span><button type="button" disabled={busy} onClick={() => changeMode(mode === "signin" ? "signup" : "signin")} className="h-9 cursor-pointer rounded-lg border border-border bg-muted/40 px-4 font-medium text-foreground transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50">{mode === "signin" ? "Sign up" : "Sign in"}</button></div>

...
const list = (...names) => names.map((name, index) => ({ id: `p${index + 1}`, name, server: "NA" }))
...
test("createProfile appends, rejects case-insensitive duplicates, and caps at 10", () => {
  assert.deepEqual(createProfile(list("Main"), " JP alt ", "new"), [...list("Main"), { id: "new", name: "JP alt", server: "NA" }])
  assert.throws(() => createProfile(list("Main"), "main", "new"), /already/)
  const full = Array.from({ length: MAX_PROFILES }, (_, i) => ({ id: `p${i}`, name: `Alt ${i}`, server: "NA" }))
...
test("loadGuestProfiles moves the legacy guest save into Main once", () => {
  ...
  assert.deepEqual(loaded, { version: 1, active: "main-id", profiles: [{ id: "main-id", name: "Main", server: "NA" }] })
...
test("planImport skips empty profiles, renames clashes, and stops at the cap", () => {
  ...
  assert.deepEqual(plan.add, [{ id: "g1", name: "Main (device)", server: "NA" }, { id: "g3", name: "JP", server: "NA" }])
  assert.deepEqual(plan.skippedEmpty.map((p) => p.id), ["g2"])
  assert.deepEqual(plan.skippedOverCap.map((p) => p.id), ["g4"])
})

test("describeProfileError turns database errors into messages", () => {
  assert.equal(describeProfileError({ code: "23505" }, "JP"), "A profile named “JP” already exists.")
  assert.equal(describeProfileError({ code: "P0001", message: "Profile limit reached" }), "You can have up to 10 profiles.")
  assert.equal(describeProfileError({ code: "P0001", message: "Cannot delete last profile" }), "You can't delete your last profile.")
  assert.equal(describeProfileError({ code: "P0002" }), "That profile no longer exists.")
  assert.equal(describeProfileError(new ProfileError("Enter a profile name.")), "Enter a profile name.")
  assert.match(describeProfileError(new TypeError("Failed to fetch")), /connection/)
})

test("profiles carry a game server; older lists default to NA", async () => {
  const { setProfileServer, pickProfileForServer } = await import("./profiles.ts")
  assert.deepEqual(createProfile([], "JP alt", "j", "JP"), [{ id: "j", name: "JP alt", server: "JP" }])
  assert.deepEqual(createProfile([], "Main", "m"), [{ id: "m", name: "Main", server: "NA" }])
  const store = memoryStore()
  saveGuestProfiles(store, { version: 1, active: "a", profiles: [{ id: "a", name: "Main" }] })
  assert.deepEqual(loadGuestProfiles(store, ids("x")).profiles, [{ id: "a", name: "Main", server: "NA" }])
  assert.equal(loadGuestProfiles(memoryStore(), ids("m")).profiles[0].server, "NA")
  assert.deepEqual(setProfileServer([{ id: "a", name: "Main", server: "NA" }], "a", "JP"), [{ id: "a", name: "Main", server: "JP" }])
  assert.throws(() => setProfileServer([], "a", "JP"), ProfileError)

  const list = [{ id: "n1", name: "Main", server: "NA" }, { id: "j1", name: "JP", server: "JP" }, { id: "j2", name: "JP2", server: "JP" }]
  assert.equal(pickProfileForServer(list, "JP", "j2"), "j2", "last used of that server")
  assert.equal(pickProfileForServer(list, "JP", "n1"), "j1", "remembered id is the wrong server")
  assert.equal(pickProfileForServer(list, "JP", null), "j1")
  assert.equal(pickProfileForServer([list[0]], "JP", null), null)
})

test("planImport keeps each device profile's server", () => {
  const plan = planImport([{ id: "g1", name: "JP", server: "JP", hasProgress: true }], [])
  assert.deepEqual(plan.add, [{ id: "g1", name: "JP", server: "JP" }])
})

...
import { EMPTY_FILTERS, countActiveFilters, filterServants, searchServants, sortServants } from "./servant-filters.ts"
...
const NO_CONTEXT = { query: "", trackedIds: [] }
...
test("collection filter uses tracked ids", () => {
  const context = { ...NO_CONTEXT, trackedIds: [300] }
  assert.deepEqual(ids(filterServants(SERVANTS, { ...EMPTY_FILTERS, collection: ["tracked"] }, context)), [300])
  assert.deepEqual(filterServants(SERVANTS, { ...EMPTY_FILTERS, collection: ["tracked"] }, NO_CONTEXT), [])
})
...
test("searchServants ranks name prefix, then word prefix, then substring, then class", () => {
  const servants = [
    { id: 1, name: "Nero Claudius", className: "Saber", rarity: 4 },
    { id: 2, name: "Altria Pendragon (Lancer)", className: "Lancer", rarity: 5 },
    { id: 3, name: "Altria Pendragon", className: "Saber", rarity: 5 },
    { id: 4, name: "Mysterious Heroine X (Alter)", className: "Berserker", rarity: 5 },
    { id: 5, name: "Salter", className: "Ruler", rarity: 4 },
  ]

  assert.deepEqual(ids(searchServants(servants, "alt")), [3, 2, 4, 5])
  assert.deepEqual(ids(searchServants(servants, "saber")), [1, 3])
  assert.deepEqual(ids(searchServants(servants, "  ")), [])
  assert.deepEqual(ids(searchServants(servants, "alt", 2)), [3, 2])
})
